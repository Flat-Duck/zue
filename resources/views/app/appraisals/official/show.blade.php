...
<div class="container-xl">
    <div class="page-header d-print-none">
        <div class="row align-items-center">
            <div class="col">
                <h2 class="page-title">نتيجة تقييم الأداء (Official Result)</h2>
                <div class="text-muted">النتيجة النهائية للموظف</div>
            </div>
            <div class="col-auto ms-auto d-print-none gap-2 d-flex">
                ...
                <form method="POST" action="{{ route('appraisals.official.finalize', [$period->id, $employee->id]) }}">
                    @csrf
                    <button class="btn btn-primary">اعتماد النتيجة / Finalize</button>
                </form>
                @endif
            </div>
        </div>
    ...
    @if(!$official)
    <div class="alert alert-warning mt-3">يجب اعتماد النتيجة (Finalize) لعرض التقرير النهائي.</div>
    @else
    <!-- Report Container -->
    <div class="card mt-3 border-2" style="border-color: #000;">
        <div class="card-body p-5">

            <!-- Header -->
            <div class="row mb-5 text-center">
                <div class="col-4 text-start">
                    <img src="https://via.placeholder.com/150x50?text=LOGO" alt="Logo" style="height: 50px;">
                </div>
                <div class="col-4">
                    <h2 class="mb-1 text-uppercase text-decoration-underline">تقرير تقييم الأداء</h2>
                    <h4 class="text-muted">{{ $period->label }}</h4>
                </div>
            ...
            <!-- 1. Employee Info -->
            <h3 class="mb-3 text-uppercase bg-light p-2 border">1. بيانات الموظف (Employee Info)</h3>
            <table class="table table-bordered mb-4">

            </table>

            <!-- 2. Attendance -->
            <h3 class="mb-3 text-uppercase bg-light p-2 border">2. بيانات الدوام (Attendance)</h3>
            <table class="table table-bordered mb-4 text-center">
                <thead class="bg-light">
                    <tr>
                        <th>أجازات مرضية (Sick Leave)</th>
                        <th>غياب (Absent)</th>
                        <th>أجازة بدون راتب (Unpaid)</th>
                        <th>الجزاءات (Penalties)</th>
                    </tr>
                </thead>
                <tbody>
                    <tr>
                        <td>{{ $attendanceStats['sick_leaves'] }}</td>
                        <td>{{ $attendanceStats['absence_days'] }}</td>
                        <td>{{ $attendanceStats['unpaid_leaves'] }}</td>
                        <td>{{ $attendanceStats['penalties'] }}</td>
                    </tr>
                </tbody>
            </table>

            <!-- 3. Evaluation Scores -->
            <h3 class="mb-3 text-uppercase bg-light p-2 border">3. عناصر التقييم (Evaluation Items)</h3>
            <table class="table table-bordered table-sm mb-4">
                <thead class="bg-light text-center">
                    <tr>
                        <th style="width: 5%">#</th>
                        <th style="width: 75%" class="text-start">عنصر التقييم (Item)</th>
                        <th style="width: 10%">الدرجة القصوى</th>
                        <th style="width: 10%">الدرجة المستحقة</th>
                    </tr>
                </thead>
                <tbody>
                    @foreach($official->scores as $index => $s)
                    @php $vi = $s->formVersionItem; @endphp
                    <tr>
                        <td class="text-center">{{ $index + 1 }}</td>
                        <td>{{ $vi->resolved_label }}</td>
                        <td class="text-center">{{ $vi->resolved_max_score }}</td>
                        <td class="text-center fw-bold">{{ $s->avg_score }}</td>
                    </tr>
                    @endforeach
                    <tr class="bg-light fw-bold fs-4">
                        <td colspan="2" class="text-end">المجموع الكلي (Total Score)</td>
                        <td class="text-center">{{ $official->max_score }}</td>
                        <td class="text-center">{{ $official->total_score }}</td>
                    </tr>
                    <tr class="bg-light fw-bold fs-4">
                        <td colspan="2" class="text-end">النسبة المئوية (Percentage)</td>
                        <td colspan="2" class="text-center text-primary">{{ $official->percentage }}%</td>
                    </tr>
                    <tr class="bg-light fw-bold fs-4">
                        <td colspan="2" class="text-end">التقدير العام (Grade)</td>
                        <td colspan="2" class="text-center {{ match ($official->grade) {
                'ممتاز' => 'text-success',
                'جيد جداً' => 'text-teal',
                'جيد' => 'text-info',
                'مقبول' => 'text-warning',
            ...
            <!-- 4. Signatures -->
            <h3 class="mb-3 text-uppercase bg-light p-2 border">4. التوقيعات (Signatures)</h3>
            <div class="row text-center mt-5">
                <div class="col-4">
                    <div class="mb-4 fw-bold">الموظف (Employee)</div>
                    <br>
                    <div class="border-top w-50 mx-auto pt-2">التوقيع / Signature</div>
                </div>
                <div class="col-4">
                    <div class="mb-4 fw-bold">المدير المباشر (Direct Manager)</div>
                    <br>
                    <div class="border-top w-50 mx-auto pt-2">التوقيع / Signature</div>
                </div>
                <div class="col-4">
                    <div class="mb-4 fw-bold">الموارد البشرية (HR)</div>
                    <br>
                    <div class="border-top w-50 mx-auto pt-2">التوقيع / Signature</div>
                </div>
            </div>
