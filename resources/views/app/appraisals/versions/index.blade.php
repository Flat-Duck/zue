    ...
    <div class="container-xl">
        <div class="page-header d-print-none">
            <div class="row align-items-center">
                <div class="col">
                    <h2 class="page-title">Versions</h2>
                    <div class="text-muted">{{ $form->name_ar }} — {{ $form->code }}</div>
                </div>
                <div class="col-auto ms-auto d-print-none">
                    <a href="{{ route('appraisals.forms.edit', $form->id) }}" class="btn btn-outline-primary">Edit
                        Form</a>
                </div>
            </div>
        </div>
                    ...
                    <tbody>
                        @foreach($versions as $v)
                            <tr>
                                ...
                                <td>{!! $v->is_active ? '<span class="badge bg-green">YES</span>' : '<span class="badge bg-secondary">NO</span>' !!}
                                </td>
                                <td class="d-flex gap-2">
                                    <a class="btn btn-sm btn-outline-success"
                                        href="{{ route('appraisals.version-items.edit', $v->id) }}">Items</a>
                                    @if(!$v->is_active)
